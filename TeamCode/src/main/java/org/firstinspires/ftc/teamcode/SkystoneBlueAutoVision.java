...
@Autonomous(name="SkystoneAutoBlueVision")
public class SkystoneBlueAutoVision extends LinearOpMode {
    Robot robot;
    ...
    public void runOpMode() {
            ...
            if (!vuforia.getFrameQueue().isEmpty()) {
                ...
                for (int i = 0; i < vuFrame.getNumImages(); i++) {
                    ...
                    if (img.getFormat() == PIXEL_FORMAT.RGB565) {
                        ...
                        if (vu < 250) { pos = 0; telemetry.addData("SKYSTONE POS: ", "LEFT");}
                        else if (vu > 250 && vu < 620) { pos = 1;telemetry.addData("SKYSTONE POS: ", "MIDDLE"); }
                        else { pos = 2; telemetry.addData("SKYSTONE POS: ", "RIGHT");}
                    }
                }
            }
        ...
        if (opModeIsActive() && !isStopRequested()) {
            ...
            double offset = 0;
            if (pos == 0) {
                scheduler.setStrafeLeft();
                offset = 8;
            }
            else if (pos == 2) {
                scheduler.setStrafeRight();
                offset = -8;
            }

            //scheduler.updateAndBlockFor(5);

            scheduler.setBlueBlockPath(robot.drivetrain.getEstimatedPose().y);
            //telemetry.update();
        }
